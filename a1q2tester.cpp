...
void getAdjacentCells(int maxRow, int maxCol, Wall walls[], int numWalls,
	...
	for (int i = 0; i < numCells; i++) {
		adjacent[i] = new int[2];
		if (k < maxRow - 1) {   //k<1 true
			if (j < maxCol - 1) {  //0 < 2
				adjacent[i][0] = i + 1;  // 1		 
				adjacent[i][1] = i + maxCol;//3		 
				j++;//1
				numAdjacent[i] = 2;//
			}
			else {
				adjacent[i][0] = i + maxCol;//5
				j = 0;
				numAdjacent[i] = 1;//
				k++;//1
			}
	...
	for (int i = 0; i < numWalls; i++) {
		...
		//changed here cuz when c1 = -1, programs throw exception
		if (c1 >= 0 && c2 >= 0 && adjacent[c1][0] == c2) {
			adjacent[c1][0] = adjacent[c1][1];
			numAdjacent[c1]--;
		}
	}
...
//	int mazeSizes[10][2] = { {1,6},{6,1},{2,3},{3,2},{20,20},
//							{70,100},{50,50},{100,70},{30,60},{40,30} };
	int mazeSizes[4][2] = { {1,6},{6,1},{2,3},{3,2} };
